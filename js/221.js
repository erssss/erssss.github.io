// 容器
const container_shine=document.querySelector('.container_shine');
...
function getNumbersFromDate(){
    ...
    function updateElementPosition(x,y){
        container_shine.style.setProperty('--x',x);
        container_shine.style.setProperty('--y',y);
    }

    // 绘制动画
    function animationLoop(){
        // 判断当前DOM节点是否能完全匹配对应的CSS的hover选择器
        const isHovered=container_shine.matches(':hover');
        const rect=container_shine.getBoundingClientRect();
